  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      * {
      ...
      /* BuildPilot project/home modal styles */
      ...
      .bp-airo-new-project-modal{max-width:860px}
      .bp-airo-new-title{text-align:center;margin-bottom:18px}.bp-airo-new-title h2{font-size:32px;line-height:1.15;margin:0 0 7px;letter-spacing:-.6px}.bp-airo-new-title p,.bp-airo-upgrade-modal p,.bp-airo-requests-modal>p{color:#64748b;margin:0}
      ...
      .bp-airo-create-tabs{display:flex;justify-content:center;gap:9px;margin:4px 0 18px;padding-bottom:14px;border-bottom:1px solid #e5e7eb}
      .bp-airo-create-tab{border:1px solid #e0e0e4;background:#fff;color:#444;border-radius:999px;padding:9px 17px;font-size:13px;cursor:pointer}
      .bp-airo-create-tab.active{border:2px solid #6d49c7;color:#3f2a78;background:#fff;font-weight:700;padding:8px 16px;box-shadow:0 3px 10px rgba(109,73,199,.12)}
      .bp-airo-prompt-box{border:2px solid #c8b9df;border-radius:20px;padding:14px;box-shadow:0 0 0 5px rgba(109,73,199,.05),0 16px 35px rgba(15,23,42,.09);background:#fff}
      .bp-airo-prompt-box textarea{width:100%;min-height:125px;border:0;outline:0;resize:vertical;font-size:17px;box-sizing:border-box;color:#334155}
      .bp-airo-prompt-actions{display:flex;justify-content:flex-end;align-items:center;gap:7px}.bp-airo-icon-btn{border:0;background:#fff;font-size:20px;padding:8px;cursor:pointer;color:#64748b}.bp-airo-generate{border:0;border-radius:12px;background:#b9a5d7;color:#fff;font-weight:800;padding:12px 24px;cursor:pointer}
      .bp-airo-ideas-title{text-align:center;color:#8a8f98;font-size:13px;margin:22px 0 12px}.bp-airo-pills{display:flex;gap:7px;flex-wrap:wrap;justify-content:center}.bp-airo-pills button{border:1px solid #6d49c7;background:#fff;color:#4b2f86;border-radius:999px;padding:7px 13px;cursor:pointer}.bp-airo-pills.small button{background:#f7f4fd;border:1px solid #eee6fb;font-size:12px}.bp-airo-ideas-caption{text-align:center;color:#8a8f98;font-size:12px;margin:20px 0 8px}
      .bp-airo-new-fields,.bp-airo-upgrade-grid{display:grid;grid-template-columns:2fr 1fr 1fr;gap:10px;margin-top:20px}.bp-airo-new-fields label,.bp-airo-upgrade-grid label,.bp-airo-upgrade-modal>label{display:block;font-size:12px;font-weight:800;color:#475569;margin-bottom:5px}
      .bp-airo-limit-note{margin-top:12px;background:#f8fafc;border-radius:10px;padding:10px;color:#64748b;font-size:12px}
      .bp-airo-upgrade-modal{max-width:470px;padding:20px}
      .bp-airo-upgrade-modal h2{font-size:20px;margin:0 30px 6px 0}.bp-airo-upgrade-modal p{font-size:12px;line-height:1.45}
      .bp-airo-upgrade-grid{grid-template-columns:1fr 1fr;gap:9px;margin-top:13px}
      .bp-airo-upgrade-grid .bp-input{padding:9px 10px;border-radius:9px;font-size:12px}
      .bp-airo-upgrade-modal>label{margin-top:11px}
      .bp-airo-upgrade-modal .bp-textarea{width:100%;box-sizing:border-box;padding:9px 10px;border-radius:9px;font-size:12px;min-height:68px}
      .bp-airo-upgrade-actions{display:flex;justify-content:flex-end;gap:7px;margin-top:11px}.bp-airo-upgrade-actions .bp-airo-btn{padding:8px 11px;font-size:12px}
      .bp-airo-request-card{border:1px solid #e5e7eb;border-radius:13px;padding:13px;margin-top:9px}.bp-airo-request-card>div:first-child{display:flex;justify-content:space-between;gap:8px}.bp-airo-request-card span{font-size:11px;color:#94a3b8}.bp-airo-request-card p{font-size:12px;color:#64748b}.bp-airo-error{background:#fef2f2;color:#b91c1c;padding:12px;border-radius:10px;margin-top:12px}
      @media(max-width:700px){.bp-airo-new-project-modal,.bp-airo-upgrade-modal,.bp-airo-requests-modal{padding:20px}.bp-airo-new-fields,.bp-airo-upgrade-grid{grid-template-columns:1fr}.bp-airo-create-tabs{justify-content:flex-start;overflow:auto}.bp-project-search{width:180px}}
    `;
  ...
  async function renderHome() {
    ...
    root.innerHTML = `
      <style>
        ...
        .bp-airo-top-actions{display:flex;gap:9px;align-items:center}.bp-project-search{width:min(420px,42vw);height:38px;border:1px solid #d9dce1;border-radius:10px;background:#fff;padding:0 13px;font-size:13px;outline:none;box-sizing:border-box}.bp-project-search:focus{border-color:#9b7bd5;box-shadow:0 0 0 3px rgba(109,73,199,.10)}
        ...
        <div class="bp-airo-main">
          <header class="bp-airo-top">
            ...
            <div class="bp-airo-top-actions">
              <input id="bpProjectSearch" class="bp-project-search" type="search" placeholder="Search projects..." oninput="window.BuildPilot.searchProjects(this.value)" autocomplete="off">
  ...
  async function startProject(
    ...
    modal.innerHTML = `
      <div class="bp-airo-new-project-modal">
        ...
        <div class="bp-airo-create-tabs">
          <button type="button" class="bp-airo-create-tab active" onclick="window.BuildPilot.showToast('New Website selected')">◉&nbsp; New Website</button>
          <button type="button" class="bp-airo-create-tab" onclick="window.BuildPilot.fillProjectIdea('Rebuild my existing website with a modern responsive design')">↻&nbsp; Rebuild Site</button>
          <button type="button" class="bp-airo-create-tab" onclick="window.BuildPilot.fillProjectIdea('Build a custom website or system based on my requirements')">▤&nbsp; Other</button>
        </div>
...
.bp-starter-card button {
  ...
  async function loadProjects() {
    ...
    const query = (document.getElementById("bpProjectSearch")?.value || "").trim().toLowerCase();
    const visibleProjects = !query ? cachedProjects : cachedProjects.filter(project =>
      [project.name, project.description, project.status, project.frontend, project.backend]
        .filter(Boolean).join(" ").toLowerCase().includes(query)
    );

    if (query && !visibleProjects.length) {
      list.innerHTML = `<div class="bp-airo-empty"><div class="bp-airo-empty-icon">⌕</div><h3>No matching projects</h3><p>Try another project name or keyword.</p></div>`;
      return;
    }
  ...
  function searchProjects(query) {
    const list = document.getElementById("projectsList");
    if (!list || !cachedProjects.length) return;
    const q = String(query || "").trim().toLowerCase();
    const visibleProjects = !q ? cachedProjects : cachedProjects.filter(project =>
      [project.name, project.description, project.status, project.frontend, project.backend]
    ...
    list.innerHTML = visibleProjects.map(project => {
      ...
      return `
        <article class="bp-airo-project-card">
          <div class="bp-airo-preview">
            <div class="bp-airo-status">${project.public_enabled ? "● Published" : escapeHtml(status)}</div>
            <div class="bp-airo-browser"><div class="bp-airo-browser-bar"><i class="bp-airo-dot"></i><i class="bp-airo-dot"></i><i class="bp-airo-dot"></i></div><div class="bp-airo-browser-body"><div class="bp-airo-line"></div><div class="bp-airo-line short"></div><div class="bp-airo-blocks"><div class="bp-airo-block"></div><div class="bp-airo-block"></div><div class="bp-airo-block"></div></div></div></div>
          </div>
          <div class="bp-airo-project-body">
            <div class="bp-airo-project-head"><h3 class="bp-airo-project-name" title="${escapeAttribute(project.name || "Untitled Project")}">${escapeHtml(project.name || "Untitled Project")}</h3><button class="bp-airo-menu" title="Open project" onclick="window.BuildPilot.openProject('${escapeAttribute(project.id)}')">⋯</button></div>
            <p class="bp-airo-description">${escapeHtml(project.description || "No description added yet.")}</p>
            <div class="bp-airo-meta"><span>${escapeHtml(date)} ${project.public_enabled ? '<b class="bp-airo-public"> · Public</b>' : ''}</span><div style="display:flex;gap:7px;align-items:center;flex-wrap:wrap;justify-content:flex-end"><button class="bp-airo-open" onclick="window.BuildPilot.openProject('${escapeAttribute(project.id)}')">Open →</button><button class="bp-airo-btn" style="padding:7px 10px;font-size:11px" onclick="window.BuildPilot.openCustomerRequests('${escapeAttribute(project.id)}')">Customer Requests</button></div></div>
          </div>
        </article>`;
    }).join("");
  }
  ...
  async function sendUpgradeRequest() {
    ...
    try {
      ...
      if (pendingRequest?.length) {
        ...
        showToast("A request is already pending with Admin.", "info");
        return;
      }
